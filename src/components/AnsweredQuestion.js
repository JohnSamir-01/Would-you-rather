...
function AnsweredQuestion(props) {
   const {question,author,answer}= props;
    ...
    const optionTwoCount = optionTwo.votes.length;
		...
		const optionOnePercentage = Math.round((optionOneCount / totalVotes) * 100);
		const optionTwoPercentage = Math.round((optionTwoCount / totalVotes) * 100);
    ...
    return (
        ...
        <div className="bar">
        {answer==='optionTwo' ? ` people who Chose Your Answer : ${optionTwoCount}` : `people who Chose the other Answer : ${optionTwoCount}`}  <div className="progress progress-info active" style={{marginBottom:20,height:20,width: optionTwoPercentage +'%',minWidth:20 , backgroundColor:'pink'}}>|{optionTwoPercentage}%</div>
        </div>
